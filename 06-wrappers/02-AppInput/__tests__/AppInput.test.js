...
const findInput = (wrapper) => wrapper.findComponent({ ref: 'input' });

describe('wrappers/AppInput', () => {
  describe('AppInput', () => {
    it('AppInput должен иметь параметры value, small, rounded и multiline', () => {
      const wrapper = shallowMount(AppInput);
      expect(wrapper.vm.$options.props.value).toBeTruthy();
      expect(wrapper.vm.$options.props.small).toBeTruthy();
      expect(wrapper.vm.$options.props.rounded).toBeTruthy();
      expect(wrapper.vm.$options.props.multiline).toBeTruthy();
    });

    it('AppInput должен рендерить поле ввода с ref="input"', () => {
      const wrapper = shallowMount(AppInput);
    ...
    it('AppInput должен рендерить поле ввода с классом form-control внутри блока input-group', () => {
      ...
      expect(wrapper.find('.input-group input.form-control').exists()).toBe(true);
    });

    it('AppInput должен иметь класс form-control_sm только с параметром small', async () => {
      const wrapper = shallowMount(AppInput);
      expect(findInput(wrapper).classes('form-control_sm')).toBe(false);
      await wrapper.setProps({ small: true });
      expect(findInput(wrapper).classes('form-control_sm')).toBe(true);
    });

    it('AppInput должен иметь класс form-control_rounded только с параметром rounded', async () => {
      const wrapper = shallowMount(AppInput);
      expect(findInput(wrapper).classes('form-control_rounded')).toBe(false);
      await wrapper.setProps({ rounded: true });
      expect(findInput(wrapper).classes('form-control_rounded')).toBe(true);
    });
    ...
    it('Параметры small и rounded должны работать с многострочным AppInput', async () => {
      ...
      const textarea = findInput(wrapper);
    ...
    it.each(['password', 'num', 'range', 'date', 'time', 'email', 'tel'])(
      ...
      (type) => {
        ...
        expect(findInput(wrapper).attributes('type')).toBe(type);
      },
    );

    it.each([false, true])('AppInput должен выводить текущее значение (multiline=%s)', async (multiline) => {
      const value = 'SampleText';
      const wrapper = shallowMount(AppInput, { propsData: { multiline } });
      await wrapper.setProps({ value });
      expect(findInput(wrapper).element.value).toBe(value);
    });
