...
const MeetupAgendaItemForm = require(getSolutionPath('components/MeetupAgendaItemForm')).default;
...
describe('forms/GeneratedForm', () => {
  describe('GeneratedForm', () => {
    ...
    it('MeetupAgendaItemForm должен выводить поля доклада с корректными значениями по умолчанию', () => {
      const inputs = wrapper.findAll('input, select, textarea').wrappers.map((wrapper) => wrapper.element);
    ...
    it.each(['registration', 'opening', 'break', 'coffee', 'closing', 'afterparty'])(
      ...
      async (type) => {
        ...
        expect(inputs[3].find('label').text()).toBe('Нестандартный текст (необязательно)');
        expect(inputs[3].find('input').exists());
      },
    );
