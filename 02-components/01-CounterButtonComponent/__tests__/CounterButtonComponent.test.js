...
const { default: CounterButton } = require(getSolutionPath('CounterButton'));
...
describe('components/CounterButtonComponent', () => {
  describe('CounterButton', () => {
    ...
    it('CounterButton должен порождать событие increment с увеличенным на 1 значением по клику также обновления входного параметра', async () => {
      const INITIAL_COUNT = 1;
      const NEW_COUNT = 100;
      ...
      expect(wrapper.emitted().increment).toBeTruthy();
      expect(wrapper.emitted().increment.length).toBe(1);
      expect(wrapper.emitted().increment[0]).toEqual([NEW_COUNT + 1]);
    });
  });
});
